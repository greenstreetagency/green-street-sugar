...
define(['jquery', 'modules/constants/request', 'modules/core/cookies'], function($j, RequestConstants, Cookies) {
    var DEFAULT_FRICTION, DESKTOP_WIDTH, DOWN_KEY, LOGO_OFFSET_TOP_NARROW, LOGO_OFFSET_TOP_WIDE, PROPERTIES, STEP_SPEEDBUMP_RADIUS, SWIPE_DISTANCE, UP_KEY, VIDEO_URL, initialize;
    ...
    initialize = function(page_variant) {
        var $backedupSection, $body, $content, $conversationsSection, $coverSection, $device, $deviceWrapper, $doc, $downloadButtons, $downloadLink, $endingSection, $html, $memoriesSection, $sections, $video, $window, addMissingValues, animateStep, animationHandler, animationIntervalID, bodyHeight, cacheCSS, calcAnimationValue, calcFrictionModifier, conversationDelayID, cssCache, durationMultiplier, easeInOut, easeOut, endingTop, friction, getDefaultValueForKey, getUniqueAnimatedSelectors, handleBreakpoint, hide_video, init, isAndroid, isAnimating, isCSSDiff, isDragging, isMobile, isPhone, isScrolling, isTablet, keydownHandler, logoOffsetTop, prevScrollY, prevWindowHeight, resizeHandler, scrollHandler, scrollRatio, scrollY, scrollYDiff, setLogoOffsetTop, setScrolling, setStep, show_video, startAnimating, step, steps, touchStartY, updateEnding, updateScrollStep, updateScrollY, updateScrolling, updateStepPositions, video_clicked, windowHeight;
        $doc = $j(document);
        $window = $j(window);
        $html = $j('html');
        $body = $j('body');
        $content = $j('#content');
        $downloadLink = $j('.download-link');
        $downloadButtons = $j('.download-button');
        $sections = $j('.section');
        $coverSection = $j('#cover-section');
        $memoriesSection = $j('#memories-section');
        $conversationsSection = $j('#conversations-section');
        $backedupSection = $j('#backedup-section');
        $endingSection = $j('#ending-section');
        $deviceWrapper = $j('#device-wrapper');
        $device = $j('#device');
        $video = $j('#video');
        cssCache = {};
        isAnimating = null ;
        ...
        isDragging = false;
        isPhone = page_variant === 'mobile';
        isTablet = page_variant === 'static';
        ...
        getUniqueAnimatedSelectors = function() {
            var _step, animatedSelectors, animation, j, k, len, len1, ref;
            animatedSelectors = [];
            for (j = 0,
            len = steps.length; j < len; j++) {
                _step = steps[j];
                ref = _step.animations;
                for (k = 0,
                len1 = ref.length; k < len1; k++) {
                    animation = ref[k];
                    animation.$el = $j(animation.selector);
                    if ($j.inArray(animation.selector, animatedSelectors) === -1) {
        ...
        ;
        init = function() {
            var _step, animation, animationExists, j, k, l, len, len1, len2, prevAnimation, ref, ref1, selector;
            ref = getUniqueAnimatedSelectors();
            for (j = 0,
            len = ref.length; j < len; j++) {
                selector = ref[j];
                prevAnimation = null ;
                for (k = 0,
                len1 = steps.length; k < len1; k++) {
                    _step = steps[k];
                    animationExists = false;
                    ref1 = _step.animations;
                    for (l = 0,
        ...
        addMissingValues = function(animation, prevAnimation) {
            var j, len, prop, value;
            for (j = 0,
            len = PROPERTIES.length; j < len; j++) {
                prop = PROPERTIES[j];
                ...
                } else if (!$j.isArray(animation[prop])) {
                    if (prevAnimation) {
                        value = prevAnimation[prop][1];
                    } else {
                        value = getDefaultValueForKey(prop);
        ...
        updateScrollStep = function() {
            var _step, i, j, len, results, stepNum;
            results = [];
            for (i = j = 0,
            len = steps.length; j < len; i = ++j) {
                _step = steps[i];
                stepNum = i + 1;
        ...
        animateStep = function(step, _ratio) {
            var _step, animation, endScroll, j, len, opacity, ratio, ref, results, rotation, scale, startScroll, transform, translateX, translateY, unit;
            _step = steps[step - 1];
            ...
            len = ref.length; j < len; j++) {
                ...
                if (isCSSDiff(animation.selector, transform, opacity)) {
                    ...
                    results.push(cacheCSS(animation.selector, transform, opacity));
                } else {
                    results.push(void 0);
                }
            }
        ...
        ;
        show_video = function() {
            $j('#modal').addClass('is-modal is-video');
        ...
        ;
        hide_video = function(e) {
            if ((e != null  ? e.target : void 0) !== $video[0]) {
                $j('#modal').removeClass('is-modal is-video');
                return $video.html('');
            }
        }
        ;
        ...
        if (isPhone) {
            $downloadLink.click(function(e) {
                e.preventDefault();
                return window.location = $j('.download-button:visible').attr('href');
            }
            );
            ...
            $doc.on('touchend', function(e) {
                ...
                if (isDragging && $j(e.target).hasClass('image-item')) {
                    if (step < 6) {
                        return setStep(step + 1);
                    } else {
                        return setStep(5);
        ...
        $j("#locale-picker-link").click(function() {
            return $j("#locale-picker-modal").addClass("is-modal");
        }
        );
        $j("#locale-picker-modal, #locale-picker-modal .modal-close").click(function() {
            return $j("#locale-picker-modal").removeClass("is-modal");
        }
        );
        $j("#locale-picker-modal .modal-window").click(function() {
            return false;
        }
        );
        return $j(".locale-option").click(function(e) {
            var $form;
            return $form = $j("<form/>", {
                action: "/set_locale",
                method: "POST"
                ...
                type: "hidden",
                name: "locale",
                value: $j(e.target).data("locale")
            })).append($j("<input/>", {
                type: "hidden",
                name: "t",
                value: Cookies.read(RequestConstants.JS_CSRF_COOKIE)
